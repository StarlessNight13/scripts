...
// @version      1.0
// @description  Auto-fetches next chapters for various novel reading sites
// @author       You
...
(function() {
    ...
    class ChapterContentManager {
        ...
        getElementStyle() {
            const element = document.querySelector(this.config.selectors.content);
            if (element) {
    ...
    class ChapterLoader {
        ...
        async processChapterContent(doc) {
            const contentElement = doc.querySelector(this.config.selectors.contentContainer);
            ...
            if (window.location.hostname === 'kolnovel.org') {
                ...
                classesToRemove.forEach(className => {
                    contentElement.querySelectorAll(`.${className}`).forEach(el => el.remove());
                });
            }
        ...
        async appendChapter(content) {
            const container = this.contentManager.createChapterContainer(content, this.nextChapterUrl);
